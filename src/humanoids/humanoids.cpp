...
OpenRAVE::Vector GetZMPVector(const KinBody::Link& link){
  OpenRAVE::Vector v = link.GetTransform()*OpenRAVE::Vector(0.045,0,0);
  return v;
}
...
ZMPOneFootConstraint::ZMPOneFootConstraint(RobotAndDOFPtr rad, vector<KinBody::LinkPtr> planted_links, const VarVector& vars) :
          m_rad(rad), m_vars(vars), planted_links_(planted_links) {

  // find the equations representing the polygon
  // PolygonToEquations(m_pts, m_ab, m_c);
}

DblVec ZMPOneFootConstraint::value(const DblVec& x) {
  m_rad->SetDOFValues(getDblVec(x,m_vars));
  OR::Vector moment(0,0,0);
  // calculate center of mass
  float totalmass = 0;
  ...
  xy(0) = moment.x;
  xy(1) = moment.y;
  PolygonToEquations(GetFeetPoly_Munip(planted_links_), m_ab, m_c);
  return toDblVec(m_ab * xy + m_c);
}

ConvexConstraintsPtr ZMPOneFootConstraint::convex(const DblVec& x, Model* model) {
  DblVec curvals = getDblVec(x, m_vars);
  m_rad->SetDOFValues(curvals);
  DblMatrix jacmoment = DblMatrix::Zero(3, curvals.size());
  OR::Vector moment(0,0,0);
...
void ZMPOneFootConstraint::Plot(const DblVec& x, OR::EnvironmentBase& env, std::vector<OR::GraphHandlePtr>& handles) {
  MatrixX2d m_pts = GetFeetPoly_Munip(planted_links_);
  PolygonToEquations(m_pts, m_ab, m_c);
  Matrix<float, Dynamic, 3, RowMajor> xyz(m_ab.rows()*2, 3);
  xyz.col(2).setZero();
  int npts = m_pts.rows();
...
struct ZMPOneFootConstraintMunipInfo : public TermInfo, public MakesConstraint{
  int timestep;
  vector<string> planted_link_names;
  void fromJson(const Value& v) {
    FAIL_IF_FALSE(v.isMember("params"));
  ...
  void hatch(TrajOptProb& prob) {
    ...
    prob.addConstraint(ConstraintPtr(new ZMPOneFootConstraint(rad, planted_links, prob.GetVarRow(timestep))));
    prob.getIneqConstraints().back()->setName(name);    
  }
  DEFINE_CREATE(ZMPOneFootConstraintMunipInfo)
};
...
TRAJOPT_API void RegisterHumanoidCostsAndCnts() {
  ...
  TermInfo::RegisterMaker("zmp_onefoot",&ZMPOneFootConstraintMunipInfo::create);
}
